...
class RDPruner:
    ...
    def __call__(self, model, model_pruning,amount, args, val_loader, container, iter_start,output_dir, sparsity=None,to_prune_layerids=None, epoch_cnt=0):
        if not hasattr(self, "amount"):     # initialize at first iter
            ...
            for param in model.parameters():
                if param.requires_grad:
                    # 把所有权重值相加
                    total_params += param.data.sum()
            # total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)

            print('whole model weight',total_params)
        ...
        for k, v in model.state_dict().items():
            ...
            if "weight_orig" in k:
                count_zeros = torch.sum(v == 0).item()
                mask3+=count_zeros
        print('before pruning mask',mask,mask2,mask3)
        ...
        for k, v in model.state_dict().items():
            ...
            if "weight" in k:
                count_zeros = torch.sum(v == 0).item()
                mask2+=count_zeros
        print('after pruning mask',mask,mask2)
...
@torch.no_grad()
def _count_whole_model_weights(model):
    """
    Return a 1-dimensional tensor of #total weights.
    """
    wlist = get_whole_weight(model)
    numels = []
    for w in wlist:
