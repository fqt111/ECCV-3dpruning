...
def train(model,optpack,train_loader,test_loader,print_steps=-1,log_results=False,log_path='log.txt', ori_model=None):
    model.train()
    ...
    print(f'Before retrain test acc: {test_acc:.2f}, sparsity: {get_model_sparsity(model)}')
    ...
    while True and num_steps > 0:
        for i,data in enumerate(train_loader):
            ...
            if print_steps != -1 and training_step%print_steps == 0:
                ...
                print(f"Steps: {training_step}/{num_steps} trainloss: {loss} celoss: {celoss}")
